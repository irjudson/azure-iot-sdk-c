...
static DEVICE_HANDLE create_and_start_and_crank_device(DEVICE_CONFIG* config, time_t current_time)
{
	...
	crank_device_do_work(handle, config, current_time, DEVICE_STATE_STARTING, AUTHENTICATION_STATE_STARTED, AUTHENTICATION_STATE_STARTED);
...
// Tests_SRS_DEVICE_09_051: [If `handle`, `message` or `on_device_d2c_event_send_complete_callback` is NULL, device_send_event_async shall return a non-zero result]
...
TEST_FUNCTION(messenger_send_async_NULL_callback)
{
	...
	// act
	int result = device_send_event_async(handle, TEST_IOTHUB_MESSAGE_LIST, NULL, TEST_ON_DEVICE_EVENT_SEND_COMPLETE_CONTEXT);
...
// Tests_SRS_DEVICE_09_064: [The user callback and context saved in `send_task` shall be invoked passing the device `event_send_complete`]
